...
class RolloutResult(pydantic.BaseModel):
    ...
    def from_problem_completion(
        ...
        if choice.logprobs is not None:
            logprobs = [
                TokenSample(token=lp.token.split(":")[-1], logprob=lp.logprob) for lp in choice.logprobs.content
            ]
...
class TrainingContext(pydantic.BaseModel):
    ...
    def update_vllm_policy(self):
        ...
        if dist.get_rank() == 0:
            ...
            resp = httpx.post(f"{self.vllm_url}/collective_rpc", json={"method": "reload_weights"})
    ...
    @staticmethod
    def save_model(output_dir: str, fsdp_model, tokenizer, save_dtype: torch.dtype = None):
        """
    ...
    def vllm_is_ready(self) -> bool:
        ...
        server_is_ready, unexpected_error = readiness_tuple[0].item(), readiness_tuple[1].item()
        if unexpected_error:
