...
def _lpt_distribute_heap(
    heap: np.ndarray, heap_id: np.ndarray, lengths: np.ndarray, indices: np.ndarray, n: int64, rank: int64
) -> np.ndarray:
...
def _batch_to_minibatches_lpt_core(lengths: np.ndarray, max_tokens: int64, num_ranks: int64, rank: int64) -> tuple:
    ...
    while start_idx < n_sequences:
        ...
        left = 1
        right = min(remaining + 1, 1 + np.searchsorted(lengths_cumsum[start_idx:], s + max_tokens * num_ranks, "right"))
        ...
        if n_to_assign < num_ranks and n_to_assign < remaining:
            ...
            # Verify this works
            if not _lpt_check_heap(heap, sorted_lengths[start_idx : start_idx + n_to_assign], max_tokens, num_ranks):
        ...
        if len(rank_sequences) > 0:
            minibatch_indices[n_minibatches, : len(rank_sequences)] = rank_sequences
            minibatch_sizes[n_minibatches] = len(rank_sequences)
        else:
            # Padding minibatch
